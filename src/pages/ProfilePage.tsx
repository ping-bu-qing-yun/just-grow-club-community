...
export function ProfilePage({ onNotice, onNavigate }: { onNotice: (message: string) => void; onNavigate?: (destination: ProfileDestination) => void }) {
  ...
  const identity = user ?? { id: 'me', name: state.profile.nickname, avatar: '/assets/avatar-me.jpg', bio: state.profile.bio, role: 'user' as const };
  ...
  return (
    <main className="page club-profile">
      ...
      {user?.role === 'admin' && <section className="club-profile-section"><h2>管理</h2><button onClick={() => onNavigate?.('admin-content')}><span className="profile-row-icon"><Shield size={18} /></span><span><b>内容治理</b><small>审核、驳回、归档内容与维护标签</small></span><ChevronRight size={17} /></button></section>}
