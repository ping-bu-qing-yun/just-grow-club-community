import { ArrowLeft, ArrowUpRight, Bell, Heart, Megaphone, MessageCircle, ShieldCheck } from 'lucide-react';
...
export function NotificationDetailPage({ notification, onBack, onNavigate }: { notification: AppNotification; onBack: () => void; onNavigate: (notification: AppNotification) => void }) {
  ...
  return (
    <main className="notification-detail page">
      <header className="notification-detail__header">
        <button type="button" className="icon-button" aria-label="返回通知" onClick={onBack}><ArrowLeft size={20} /></button>
        <span>通知详情</span>
        <i aria-hidden="true" />
      </header>
      <section className="notification-detail__body">
        ...
        {canNavigate ? <button type="button" className="notification-detail__link" onClick={() => onNavigate(notification)}>{notification.target?.label ?? '查看相关内容'}<ArrowUpRight size={17} /></button> : <div className="notification-detail__note"><Bell size={17} />这条提醒会保留在通知中心，方便你随时查看。</div>}
      </section>
    </main>
  );
}
